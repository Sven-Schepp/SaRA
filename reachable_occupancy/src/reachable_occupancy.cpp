...
//! \brief Callback class for joint data
class JointReception {
  public:
    //! \brief Contains the current Cartesian joint positions
    std::vector<reach_lib::Point> positions = {};

    //! \brief Contains the current Cartesian joint velocities
    std::vector<reach_lib::Point> velocities = {};

    //! \brief Contains the time between frames
    double delta_t_ = 0.0;

    //! \brief Contains the time of the last frame
    double time_ = 0.0;

    void callback(reachable_occupancy::JointLocations data) {
      this->positions = {};
      this->velocities = {};
      this->delta_t_ = 0.0;

      // Get duration between this and last frame
      double delta_t_ = 0.0;
      ...
      if (this->time_ == 0.0) {
        ...
        delta_t_ = now - this->time_;
        this->time_  = now;
      }

      // Get current Cartesian joint positions and velocities
      for (const auto& it : data.positions) {
        this->positions.push_back(reach_lib::Point(it.x, it.y ,it.z));
      }
      for (const auto& it : data.velocities) {
        this->velocities.push_back(reach_lib::Point(it.x, it.y ,it.z));
      }
    }
};
...
int main(int argc, char** argv) {
  ...
  std::string joint_topic = "udp/p8080";
  ...
  std::vector<double> extremity_max_v = {2.46, 2.46};
  std::vector<double> extremity_length = {0.635, 0.635};
  std::vector<double> extremity_thickness = {0.205, 0.205};
  ...
  for (int i = 0; i < articulated_thickness.size(); i++) {
  ...
  while(ros::ok()) {
    ...
    if (use_visualization) {
      if (vis_joints && tsv.get_joint_pos().size() > 0) {
    ...
    if (!vis_validated && use_visualization) {
      ...
      // Robot occupancies
      if (tsv_rob.get_robot_capsules().size() > 0 && use_robot_capsules) {
    ...
    if (use_visualization) {
      ...
      if (use_robot_capsules && tsv.get_robot_capsules().size() > 0) {
