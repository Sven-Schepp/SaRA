...
int main(int argc, char** argv) {
  ...
  thickness_demo["Torso"] = 0.32;
  thickness_demo["Head"] = 0.3;
  ...
  reach_lib::ArticulatedAccel human_demo_ta = reach_lib::ArticulatedAccel(s_demo, index_demo, thickness_demo, max_a_demo);
  reach_lib::ArticulatedVel human_demo = reach_lib::ArticulatedVel(s_demo, index_demo, thickness_demo, max_v_demo);
  reach_lib::ArticulatedPos human_demo_tp = reach_lib::ArticulatedPos(s_demo, index_p_demo, thickness_p_demo, max_v_p_demo, length_demo);
  ...
  // Subscribers for translated data
  Receiver rec;
  ros::Subscriber trans_frame_data_subscriber = nh.subscribe("/SaRA_demo_human_capsules", 1000,
                                          &Receiver::human_callback, &rec);

  ros::Subscriber trans_robot_subscriber = nh.subscribe("/SaRA_demo_robot_capsules", 1000,
                                     &Receiver::robot_callback, &rec);
  ...
  // t_brake measures
  long double max_t_brake = 0.0;
  long double avg_t_brake = 0.0;
  long double sum_t_brake = 0.0;
  long double dev_t_brake = 0.0;
  ...
  while (ros::ok()) {
    ...
    // current t_brake
    if (tsv_rob.get_t_brake() > 0.0) {
      t_b = tsv_rob.get_t_brake() + 0.04; 
      if (t_b > max_t_brake) {
        max_t_brake = t_b;
        sum_t_brake += t_b;
        if (counter > 0) {
          long double old_avg_t_brake = avg_t_brake;
          avg_t_brake = sum_t_brake/static_cast<long double>(counter);
          dev_t_brake = (dev_t_brake * dev_t_brake + (1.0/static_cast<long double>(counter))
                      * ((t_b - avg_t_brake) * (t_b - old_avg_t_brake) - dev_t_brake * dev_t_brake));
        }
      }
    }

    t_b = rec.get_t_brake() +0.04;
    // commented original if translated capsules are used
    if (rec.get_robot_capsules().size() > 0) {
      r_demo.vis_capsules(rec.get_robot_capsules());
    }

    // std::cout<< "New t_brake: " << t_b << "\n";
    ...
    // translated
    if (rec.get_joint_pos().size() > 0) {
      human_demo.update(t_a, t_b, rec.get_joint_pos());
      if (rec.get_robot_capsules().size() > 0) {
        human_robot_intersections = val.capsule_set_intersection(reach_lib::get_capsules(human_demo), rec.get_robot_capsules());
      }
    }

    if (tsv.get_joint_pos().size() > 10000) {
      ...
      // Articulated
      // human_demo_ta.update(t_a, t_b, tsv.get_joint_pos(), tsv.get_joint_vel());
      // human_demo.update(t_a, t_b, tsv.get_joint_pos());
      // human_demo_tp.update(t_a, t_b, te);
      ...
      if (tsv_rob.get_robot_capsules().size() > 0) {
        ...
        // human_robot_intersections = val.capsule_set_intersection(reach_lib::get_capsules(human_demo_ta), tsv_rob.get_robot_capsules());
        // human_robot_intersections = val.capsule_set_intersection(reach_lib::get_capsules(human_demo_tp), tsv_rob.get_robot_capsules());
    ...
    if (timing) {
      ...
      deviation = (sum_sq/static_cast<long double>(counter)) - (average * average);
      std::cout << average << " " << longest << " " << shortest << " " << deviation << " " << d << "\n";
    }

    // translated
    if (rec.get_robot_capsules().size() > 0) {
      // std::cout << "Stop: " << tsv_rob.get_full_stop() << "\n";
      if (!rec.get_fs()) {
        int b_temp = human_robot_intersections.size();
        human_robot_intersections = {};
        for (int i = 0; i < b_temp; i++) {
          human_robot_intersections.push_back(false);
        }
      }
    }

    if (tsv_rob.get_robot_capsules().size() > 10000) {
      // std::cout << "Stop: " << tsv_rob.get_full_stop() << "\n";
      if (!tsv_rob.get_full_stop()) {
        int b_temp = human_robot_intersections.size();
        human_robot_intersections = {};
        for (int i = 0; i < b_temp; i++) {
          human_robot_intersections.push_back(false);
    ...
    // human_a.update(0.0, 0.04, pos, vel);
    // human_v.update(0.0, 0.015, pos);
    ...
    // translated
    if (rec.get_joint_pos().size() > 0) {
      auto b = human_demo.get_occupancy()[0].get_occupancy();
      std::vector<reach_lib::Capsule> trans_cap_vec = {};
      for (auto& it: human_demo.get_occupancy()) {
        trans_cap_vec.push_back(it.get_occupancy());
      }
      v_demo.vis_capsules(trans_cap_vec, human_robot_intersections); //human_robot_intersections
    }
    ...
    /*
    if (frame_pos.size() > 150) {
      frame_pos.erase(frame_pos.begin());
    }


    // Vis skeleton for CMU
    points_i.points = {};
    points_i.color = red;
    points_i.scale.x = 0.1;
    points_i.scale.y = 0.1;
    for (int i = 0; i < pos.size(); i++) {
      geometry_msgs::Point p_t;
      p_t.x = pos[i].x;
      p_t.y = pos[i].y;
      p_t.z = pos[i].z;
      points_i.points.push_back(p_t);
    }
    visualization_msgs::MarkerArray m_t;
    m_t.markers.push_back(points_i);*/
  ...
  std::cout << "Average|Deviation loop time: " << average << " " << deviation << "\n";
  if (avg_vel.size() >= 10) {
  ...
  std::cout << "Max t_brake: " << max_t_brake << "Avg t_brake: " << avg_t_brake << "Std dev t_brake: " << dev_t_brake << "\n";
  return 0;
}
